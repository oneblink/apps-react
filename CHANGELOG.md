...
- `hiddenFrom` support so form elements can be hidden from submitters, approvers, or both
...
- **[BREAKING]** `audience` is now a required prop of `<OneBlinkForm />`, `<OneBlinkFormControlled />`, `<OneBlinkAutoSaveForm />` and `<OneBlinkReadOnlyForm />`. Pass `'SUBMITTER'` when a user is submitting a form, or `'APPROVER'` when an approver is reviewing a submission.
...
- focus when navigating between pages
...
- `hideEndAdornment` to `AutocompleteDropdown`
...
- submission PDF fileame in post submission receipt modal
- form store file downloads
...
- arc gis map snapshot images

### Dependencies

- update [@oneblink/sdk-core](https://www.npmjs.com/package/@oneblink/sdk-core) to [8.7.0-beta.2](https://github.com/oneblink/sdk-core-js/blob/master/CHANGELOG.md) (from [8.6.0](https://github.com/oneblink/sdk-core-js/releases/tag/v8.6.0))
...
- esri layers order not being respected
...
- handling for ArcGIS web map layer visibility
- read-only functionality to "Form" form elements
...
- ArcGIS Web Map element to show/hide widgets based on element config
...
- **[BREAKING]** flatpickr and replaced with @mui/x-date-pickers
...
- no longer depend upon [flatpickr](https://www.npmjs.com/package/flatpickr)
...
- `.ob-button` css
- passing `captchaSiteKey` alongside token
- support for repeatable set layouts

## [7.1.0] - 2025-01-22

### Added

- task columns to data manager

## [7.0.0] - 2025-01-12

### Changed

- **[BREAKING]** supported version of `@oneblink/apps` to `^19.0.0`. The `captchaTokens` property on the options parameter for the `onSubmit` prop passed to `<OneBlinkForm />`, `<OneBlinkFormControlled />` and `<OneBlinkAutoSaveForm />` has been replaced with a property called `recaptchas`.
